...
def run_homography_coarse_move(robot: Robot, target_xy: np.ndarray) -> bool:
    ...
    if ENFORCE_WORKSPACE_LIMITS and not in_workspace(x_target, y_target):
        x_clamp, y_clamp = clamp_workspace(x_target, y_target)
        print(f"[COARSE] WARNING: target outside workspace. raw=({x_target:.1f},{y_target:.1f}), clamped=({x_clamp:.1f},{y_clamp:.1f})")
        x_target, y_target = x_clamp, y_clamp
...
def main():
    ...
    print_startup_config("homography_coarse_stereo_fine.py", OVERHEAD_INDEX, STEREO_INDEX)
    require_soft_limits_configured("homography_coarse_stereo_fine.py")
    ...
    try:
        require_robot_soft_limits_loaded(robot, "homography_coarse_stereo_fine.py")
        robot.enable(True)
        robot.init_drivers()
        claw_open(robot)
        ...
        robot.print_estimate()
        robot.move_cartesian(X_SURVEY,Y_SURVEY,Z_SURVEY,PHI_SURVEY,move_time_s=5.0)
